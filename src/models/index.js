...
config.define = {
    hooks: {
        ...
        afterUpdate: async (model, options) => {
            console.log("🚀 ~ afterUpdate: ~ model:", model)
...
//getting all the file name inside models directory
let filesName = fs.readdirSync(__dirname);


filesName = filesName.filter((file) => {
    return (
        file.indexOf(".") !== 0 &&
        file !== basename &&
        file.slice(-3) === ".js" &&
